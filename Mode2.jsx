...
import { COUNTRIES, resolveCountry, fmtTime, CONT_COLORS } from './data'

const LETTERS = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('')

// ISO alpha-2 to alpha-3 mapping for react-simple-maps
const CODE2TO3 = {
...
function WorldMap({ targetCodes, guessedCodes }) {
  ...
  // Use topojson manually
  return (
    <div style={{ width: "100%", background: "#0a1628", borderRadius: 12, overflow: "hidden", border: "1px solid #1e293b" }}>
      <MapSVG geoData={geoData} targetCodes={targetCodes} guessedCodes={guessedCodes} />
    </div>
  )
}

function MapSVG({ geoData, targetCodes, guessedCodes }) {
  const [paths, setPaths] = useState([])

  useEffect(() => {
    // Dynamically load topojson
    const script = document.createElement('script')
    script.src = 'https://cdnjs.cloudflare.com/ajax/libs/topojson/3.0.2/topojson.min.js'
    script.onload = () => {
      if (window.topojson && geoData) {
        const countries = window.topojson.feature(geoData, geoData.objects.countries)
  ...
  // Map numeric ISO to alpha-2
  const numericToAlpha2 = {}
  Object.entries(CODE2TO3).forEach(([a2]) => {
    // We need numeric -> alpha2, build from known list
  })
  ...
  return (
    <svg viewBox="0 0 800 400" style={{ width: "100%", display: "block" }}>
      ...
      {paths.map((feature, i) => {
        ...
        // Find matching country by numeric code
        const alpha2 = Object.entries(CODE2TO3).find(([, a3]) => {
          // numeric to alpha3 lookup via a hardcoded small table
          return false
        })?.[0]
        
        const d = pathToD(feature.geometry)
        const isTarget = false // placeholder
        ...
        return (
          <path
            ...
            stroke="#0a1628"
            strokeWidth="0.5"
            opacity={isTarget || isGuessed ? 1 : 0.4}
          />
        )
      })}
    </svg>
...
export default function Mode2({ user, scores, updateMode2, onBack }) {
  ...
  const remaining = letterCountries.filter(c => !guessed.includes(c.name))
  ...
  const giveUp = () => {
    setRunning(false); setDone(true)
    updateMode2(user, letter, time)
  }

  const targetCodes = letterCountries.map(c => CODE2TO3[c.code]).filter(Boolean)
  const guessedCodes = guessed.map(name => {
    const c = COUNTRIES.find(x => x.name === name)
    return c ? CODE2TO3[c.code] : null
  }).filter(Boolean)
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#030711", fontFamily: "'Georgia', serif", padding: "24px 20px" }}>
      <div style={{ maxWidth: 700, margin: "0 auto" }}>
        ...
        <div style={{ marginTop: 12, marginBottom: 12, display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ fontSize: 13, color: "#10B981" }}>■ Guessed</div>
          <div style={{ fontSize: 13, color: "#3B82F6" }}>■ Still needed</div>
        ...
        {!done ? (
          <div style={{ display: "flex", gap: 12, marginBottom: 20 }}>
            <input
              ...
              autoFocus
              style={{
                flex: 1, background: shake ? "#3f1515" : "#0f172a",
                border: "2px solid " + (shake ? "#EF4444" : "#1e293b"),
          ...
          <div style={{ marginBottom: 20, padding: 20, background: "#0f2a1e", border: "2px solid #10B981", borderRadius: 16, textAlign: "center" }}>
            <div style={{ fontSize: 24, fontWeight: 900, color: "#10B981" }}>
              {guessed.length === letterCountries.length ? "✓ Complete!" : `${guessed.length}/${letterCountries.length} named`}
            </div>
        ...
        {/* Leaderboard for this letter */}
        {letter && scores.mode2[letter] && (
          <div style={{ marginTop: 24, background: "#080f1e", border: "1px solid #1e293b", borderRadius: 16, padding: 20 }}>
            <div style={{ fontSize: 11, letterSpacing: 4, color: "#3B82F6", textTransform: "uppercase", marginBottom: 12 }}>🏅 {letter} — Leaderboard</div>
            {Object.entries(scores.mode2)
              .filter(([k]) => k === letter)
              .map(() => {
                const entries = scores.mode2holders2?.[letter] || {}
                const sorted = Object.entries(entries).sort(([,a],[,b]) => a - b)
                if (sorted.length === 0 && scores.mode2[letter]) {
                  return (
                    <div key="single" style={{ display: "flex", gap: 12, padding: "8px 0" }}>
                      <span style={{ fontSize: 20 }}>🥇</span>
                      <span style={{ flex: 1, color: "#34d399", fontWeight: 700 }}>{scores.mode2holders?.[letter] || "—"}</span>
                      <span style={{ color: "#e2e8f0", fontWeight: 700 }}>{fmtTime(scores.mode2[letter])}</span>
                    </div>
                  )
                }
                return sorted.map(([uid, t], i) => (
                  <div key={uid} style={{ display: "flex", gap: 12, padding: "8px 0", borderBottom: i < sorted.length - 1 ? "1px solid #1e293b" : "none" }}>
...
// Self-contained SVG world map component
  ...
  useEffect(() => {
    ...
    const loadMap = async () => {
      try {
        ...
        setLoaded(true) // show nothing rather than spin forever
      }
    }
  ...
  return (
    <div style={{ width: "100%", background: "#060d1a", borderRadius: 12, overflow: "hidden", border: "1px solid #1e293b" }}>
      <svg viewBox="0 0 800 400" style={{ width: "100%", display: "block" }}>
        ...
        {!loaded && (
          <text x="400" y="200" textAnchor="middle" fill="#475569" fontSize="14">Loading map...</text>
        )}
        {paths.map((p, i) => {
...
// ISO numeric to alpha-2 mapping
