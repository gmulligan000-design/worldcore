...
function WorldMap({ guessedCodes }) {
  const [paths, setPaths] = useState([])
  const [loaded, setLoaded] = useState(false)
  const allCodes = COUNTRIES.map(c => CODE2TO3[c.code]).filter(Boolean)
  useEffect(() => {
    let cancelled = false
    const load = async () => {
      try {
    ...
    <div style={{width:"100%",background:"#060d1a",borderRadius:12,overflow:"hidden",border:"1px solid #1e293b"}}>
      <svg viewBox="0 0 800 400" style={{width:"100%",display:"block"}}>
        ...
        {paths.map((p,i)=>{
          ...
          if(!d)return null
          return <path key={i} d={d} fill={ig?"#10B981":isWorld?"#1e3a5f":"#0f172a"} stroke="#060d1a" strokeWidth="0.5" opacity={ig?1:isWorld?0.5:0.2}/>
        })}
      </svg>
    </div>
  )
    ...
    <div style={{minHeight:"100vh",background:"#030711",fontFamily:"'Georgia',serif",padding:"24px 20px"}}>
      <div style={{maxWidth:700,margin:"0 auto"}}>
        ...
        <div style={{marginTop:10,marginBottom:10,display:"flex",alignItems:"center",gap:16}}>
          ...
          <div style={{fontSize:12,color:"#1e3a5f"}}>■ Remaining</div>
          <div style={{marginLeft:"auto",fontSize:13,color:"#e2e8f0",fontWeight:700}}>{guessed.length}/{COUNTRIES.length}</div>
        </div>

        {!started ? (
          <button onClick={start} style={{width:"100%",background:"linear-gradient(135deg,#1e3a5f,#0f172a)",border:"2px solid #3B82F6",color:"#60a5fa",padding:"20px",borderRadius:14,cursor:"pointer",fontSize:18,fontFamily:"inherit",fontWeight:700}}>
            Start Timer →
          </button>
        ) : !done ? (
          <div style={{display:"flex",gap:12,marginBottom:16}}>
            <input ref={inputRef} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleGuess()}
        ...
        {started && !done && (
          <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:16,maxHeight:200,overflowY:"auto"}}>
            {guessed.map(name=>{const c=COUNTRIES.find(x=>x.name===name);return(
              <div key={name} style={{padding:"4px 10px",borderRadius:6,fontSize:12,background:"#0f2a1e",border:"1px solid #10B981",color:"#10B981"}}>
                {c?.flag} {name}
              </div>
            )})}
          </div>
        ...
        {done && remaining.length > 0 && (
          <div style={{marginBottom:16}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
              {remaining.map(c=>(
                <div key={c.name} style={{padding:"4px 10px",borderRadius:6,fontSize:12,background:"#1a0f0f",border:"1px solid #EF4444",color:"#EF4444"}}>
                  {c.flag} {c.name}
                </div>
              ))}
            </div>
